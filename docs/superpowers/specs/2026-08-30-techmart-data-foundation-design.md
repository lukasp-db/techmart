...
- **Compute (revised 2026-08-30):** all generation runs **on Databricks serverless as Spark**,
  not locally. Databricks is a Spark runtime — generation is expressed in **`dbldatagen`/PySpark**
  and executed on serverless via `DatabricksSession.builder.serverless()` or notebook context.
  Every table (dims + facts) is generated on serverless and written to Unity Catalog Delta **with
  column comments**. No classic clusters or instance-specific runtime features.
  - **Notebooks + DAB:** generation ships as **Python Databricks notebooks** deployed as
    **`notebook_task`s** in the DAB and run on serverless — following the `databricks-data-generation`
    skill's three-tier model. Large facts (>5M rows) and any UDF-dependent feature
    (`distribution=Gamma/Beta/Normal`, `text=mimesisText`, `.withConstraint()`) use **Tier-3
    notebooks** (these fail over Databricks Connect); Catalyst-safe features (`values=`/`weights=`,
    `minValue`/`maxValue`, `expr=`, `begin`/`end`/`interval`, `percentNulls=`, `omit=True`,
    `explode(sequence(...))`) run anywhere. `partitions` is always set explicitly.
  - **Testable core:** the `techmart` package holds the importable generation logic (specs,
    measures, seasonality, basket assembly); notebooks are thin serverless wrappers over it, so
    the logic keeps fast local **pytest** coverage (local Spark) for the public repo / CI.
  - *Superseded:* the earlier "Polars locally for dims" approach — dimensions are **generated in
    dbldatagen/PySpark** like the facts (reference data such as the taxonomy/name pools is
    engine-agnostic and reused). Rule-based `dim_date` (fiscal 4-5-4 + holidays) is built with
    PySpark `createDataFrame` over the existing deterministic calendar logic, not sampled.
- **Deployment:** packaged as a **Databricks Asset Bundle (DAB)** (`databricks.yml`) that defines
  the serverless generation jobs (notebook tasks, dependency-ordered: dims → facts), schema/table
  creation, metric views, and materializations for one-command deploy. Introduced at Phase 3;
...
- **Basket coherence (required):** lines sharing a `transaction_id` are a real receipt — they
  carry the **same** `date_sk`, `store_sk`, `customer_sk`, `employee_sk`, and `channel_sk`, so
  `GROUP BY transaction_id` roll-ups (basket size, receipt total) are meaningful. Generated by
  building a transaction **header** (constant attrs + a weighted basket size 1–8) then
  `explode(sequence(1, basket_size))` into per-line rows.
...
- **Generation approach (revised 2026-08-30):** **`dbldatagen`/PySpark on serverless for every
  table** (dims and facts), deployed as notebook tasks via the DAB; LLM for
  `product_review`/`service_case` text. Idempotent, re-runnable, config-driven. See the revised
  **Compute** bullet in Architecture for the notebook/DAB/three-tier detail. (Supersedes the
  earlier "local Polars for dims" split.)
