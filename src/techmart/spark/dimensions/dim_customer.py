...
_FIRST_NAMES_ARR = "array(" + ", ".join(f"'{n}'" for n in FIRST_NAMES) + ")"
_LAST_NAMES_ARR = "array(" + ", ".join(f"'{n}'" for n in LAST_NAMES) + ")"
_CITIES_ARR = "array(" + ", ".join(f"'{c}'" for c in CITIES) + ")"
_STATES_ARR = "array(" + ", ".join(f"'{s}'" for s in US_STATES) + ")"
...
def build_dim_customer(spark: SparkSession, config: TechmartConfig) -> DataFrame:
    ...
    return DIM_CUSTOMER_SPEC.select_ordered(df)
